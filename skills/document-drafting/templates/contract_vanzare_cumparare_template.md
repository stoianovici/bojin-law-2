# CONTRACT DE VANZARE-CUMPARARE / SALES AGREEMENT
...
**1.2** Caracteristicile bunului / Property characteristics:
{{CARACTERISTICI_BUN}}

**1.3** Situația juridică / Legal status:
{{SITUATIE_JURIDICA}}

---

## 2. PREȚUL / PURCHASE PRICE
...
## 5. PREDARE-PRIMIRE / DELIVERY AND ACCEPTANCE
...
## 6. DECLARAȚII ȘI GARANȚII / REPRESENTATIONS AND WARRANTIES
...
## 12. CLAUZE SPECIALE / SPECIAL CLAUSES
...
- `{{VANZATOR_IDENTIFICARE}}` - ID/CUI/CNP number
- `{{CUMPARATOR_NUME}}` - Buyer's full legal name
- `{{CUMPARATOR_TIP_PERSOANA}}` - Buyer type (persoană fizică / juridică)
- `{{CUMPARATOR_ADRESA}}` - Buyer's address
...
- `{{PRET_TOTAL}}` - Total price (numerical)
- `{{PRET_IN_LITERE}}` - Price in words
...
- `{{SITUATIE_JURIDICA}}` - Legal status details
- `{{CLAUZA_TVA}}` - VAT clause
- `{{INCLUZIUNI_PRET}}` - Price inclusions
- `{{CLAUZA_AVANS}}` - Down payment clause
- `{{CLAUZA_SOLD}}` - Balance payment clause
- `{{PENALITATE_PROCENT}}` - Late payment penalty percentage (default: 0.1)
- `{{STARE_BUN}}` - Condition description
- `{{TERMEN_VICII}}` - Hidden defects notification period (default: 30)
- `{{PREAVIZ_REZILIERE}}` - Termination notice period (default: 15)
- `{{INSTANTA_COMPETENTA}}` - Competent court location
- `{{NUMAR_EXEMPLARE}}` - Number of original copies (default: 2)
