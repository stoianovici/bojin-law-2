#!/usr/bin/env node
/**
 * Sync scanned documents from local to R2
 * Only uploads files listed in /tmp/scanned-doc-ids.txt
 */
...
async function main() {
  // Read scanned document IDs from file
  const scannedDocIds = fs
    .readFileSync('/tmp/scanned-doc-ids.txt', 'utf-8')
    .split('\n')
    .filter((line) => line.trim().length > 0);

  console.log(`Scanned documents to sync: ${scannedDocIds.length}`);

  console.log('Fetching R2 file list...');
  const r2Files = await getR2Files();
  console.log(`Found ${r2Files.size} files in R2`);

  // Find missing scanned files
  const missing = scannedDocIds.filter((f) => !r2Files.has(f));
  console.log(`Missing scanned docs from R2: ${missing.length} files`);

  if (missing.length === 0) {
    console.log('All scanned documents are in sync!');
    return;
  }
