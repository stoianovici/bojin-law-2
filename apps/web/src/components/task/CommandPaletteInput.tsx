...
function SuggestionItem({ suggestion, isSelected, onClick, language }: SuggestionItemProps) {
  ...
  const taskTypeLabel = labels.taskTypes[suggestion.taskType as keyof typeof labels.taskTypes] || suggestion.taskType;
...
function HighlightedText({ text, entities }: { text: string; entities: ParsedEntity[] }) {
  ...
  sortedEntities.forEach((entity, idx) => {
    ...
    const colors = ENTITY_COLORS[entity.type] || { bg: 'bg-gray-100', text: 'text-gray-700', border: 'border-gray-300' };
    parts.push(
      <span
        key={`entity-${idx}`}
        ...
        >
          ...
          <div className="px-3 py-2 border-t border-gray-100 bg-gray-50">
            <span className="text-xs text-gray-500">
              {language === 'ro' ? 'Folosește ↑↓ pentru navigare, Tab pentru selectare' : 'Use ↑↓ to navigate, Tab to select'}
            </span>
          </div>
        </div>
      )}
