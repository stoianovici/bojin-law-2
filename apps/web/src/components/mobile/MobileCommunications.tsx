      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {caseItem.threads.length > 0 && (
          <div className="text-center py-4 text-sm text-linear-text-muted">
            {caseItem.threads.length} {caseItem.threads.length === 1 ? 'conversație' : 'conversații'}
          </div>
        )}
      </main>
    </div>
...
function CaseEmailRow({ caseItem, onTap }: CaseEmailRowProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm truncate ${caseItem.unreadCount > 0 ? 'font-semibold text-linear-text-primary' : 'font-medium text-linear-text-primary'}`}>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <p className={`text-sm truncate ${thread.hasUnread ? 'font-semibold text-linear-text-primary' : 'text-linear-text-primary'}`}>
            {senderName}
          </p>
        ...
        {/* Subject */}
        <p className={`text-sm truncate ${thread.hasUnread ? 'font-medium text-linear-text-primary' : 'text-linear-text-secondary'}`}>
          {thread.subject}
        </p>
        ...
        <div className="flex items-center gap-2 mt-1 text-xs text-linear-text-muted">
          {thread.messageCount > 1 && (
            <span>{thread.messageCount} mesaje</span>
          )}
