...
export function TutorialController() {
  ...
  // Kill switch - completely disable tutorial
  ...
  const handleChoice = useCallback((choice: 'skip' | 'continue') => {
    if (choice === 'skip') {
      completeTutorial();
    } else {
      advanceStep();
  ...
  return (
    <>
      <TutorialMask />
      {currentStep?.showChoiceDialog && (
        <TutorialChoiceDialog onChoice={handleChoice} />
      )}
    </>
  );
}
