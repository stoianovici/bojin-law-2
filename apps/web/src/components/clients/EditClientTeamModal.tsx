...
import { ASSIGN_CLIENT_TEAM, REMOVE_CLIENT_TEAM } from '@/graphql/mutations';
...
export interface EditClientTeamModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  clientId: string;
  currentTeam: ClientTeamMember[];
  onSuccess?: () => void;
}
...
// ============================================================================
// EditClientTeamModal Component
// ============================================================================

export function EditClientTeamModal({
  open,
  onOpenChange,
  clientId,
  currentTeam,
  onSuccess,
}: EditClientTeamModalProps) {
  ...
  // GraphQL mutations
  const [assignClientTeam] = useMutation(ASSIGN_CLIENT_TEAM);
  const [removeClientTeam] = useMutation(REMOVE_CLIENT_TEAM);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      for (const member of toRemove) {
        const result = await removeClientTeam({
          variables: { clientId, userId: member.user.id },
        });
      ...
      // Execute additions and updates
      for (const member of [...toAdd, ...toUpdate]) {
        const result = await assignClientTeam({
          variables: {
            clientId,
      ...
      console.error('[EditClientTeamModal] Error:', err);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="md">
        <DialogHeader>
          <DialogTitle>Editează echipa clientului</DialogTitle>
          <DialogDescription>
            Adaugă sau elimină membrii echipei responsabile de acest client.
          </DialogDescription>
        </DialogHeader>
...
EditClientTeamModal.displayName = 'EditClientTeamModal';

export default EditClientTeamModal;
