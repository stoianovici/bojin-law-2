 ...
 * Collapsible card showing team member's activity
 *
 * Features:
 * - Avatar + name header
  ...
  return (
    ...
    >
      <CollapsibleTrigger asChild>
        ...
        >
          {/* Avatar */}
          <div className="flex-shrink-0 h-10 w-10 rounded-full bg-linear-accent/20 flex items-center justify-center">
            <span className="text-sm font-medium text-linear-accent">{initials}</span>
          </div>

          {/* Name and role */}
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-linear-text-primary truncate">{displayName}</p>
            <p className="text-xs text-linear-text-muted">{user.role}</p>
          </div>

          {/* Stats badges */}
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-linear-bg-tertiary text-linear-text-secondary">
              {stats.totalTasks} {stats.totalTasks === 1 ? 'sarcină' : 'sarcini'}
            </span>
            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-linear-accent/20 text-linear-accent">
        ...
        <div className="border-t border-linear-border-subtle">
          {entries.length === 0 ? (
            <div className="px-4 py-6 text-center">
              <User className="h-8 w-8 text-linear-text-muted mx-auto mb-2" aria-hidden="true" />
              <p className="text-sm text-linear-text-muted">Nicio activitate înregistrată</p>
            </div>
          ) : (
