...
const shortcutHintVariants = cva('inline-flex items-center gap-0.5', {
  variants: {
    ...
    variant: {
      default: '[&>kbd]:bg-linear-bg-tertiary [&>kbd]:border-linear-border [&>kbd]:text-linear-text-tertiary',
      muted: '[&>kbd]:bg-transparent [&>kbd]:border-linear-border-subtle [&>kbd]:text-linear-text-muted',
      accent: '[&>kbd]:bg-linear-accent/10 [&>kbd]:border-linear-accent/30 [&>kbd]:text-linear-accent',
    },
    /** Size variant */
    size: {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between gap-4">
        ...
        {keys && keys.length > 0 && (
          <ShortcutHint keys={keys} variant="muted" size="sm" />
        )}
      </div>
      {description && (
        <span className="text-xs text-linear-text-muted">{description}</span>
      )}
    </div>
  );
}
