/**
 * ONRC Templates - Complete Document Requirements
 *
 * CRITICAL: This is the source of truth for all ONRC procedure templates.
 * Contains 58 procedures with their required documents.
 *
 * DO NOT store these in a database - they are static, official data.
 * Only firm-specific custom templates should be stored in the database.
 */
...
interface ONRCTemplateDefinition {
  procedureId: string;
  name: string;
  description: string;
  sourceUrl: string;
...
// ============================================================================
// ALL ONRC TEMPLATES (58 procedures)
// ============================================================================

const ONRC_TEMPLATE_DEFINITIONS: ONRCTemplateDefinition[] = [
  // ==========================================================================
...
/**
 * Convert internal definitions to MapaTemplate format
 */
function toMapaTemplate(def: ONRCTemplateDefinition): MapaTemplate {
  const now = new Date().toISOString();
  return {
    id: `onrc-${def.procedureId}`,
    firmId: null,
    name: def.name,
    ...
    lastSynced: now,
    contentHash: Buffer.from(JSON.stringify(def.slots)).toString('base64').substring(0, 32),
    createdAt: now,
    updatedAt: now,
    createdBy: null,
    slotDefinitions: def.slots,
    usageCount: 0,
  };
}

/**
 * All ONRC templates as MapaTemplate objects
 * This is the source of truth - no database needed
 */
export const ONRC_TEMPLATES: MapaTemplate[] = ONRC_TEMPLATE_DEFINITIONS.map(toMapaTemplate);

/**
 * Get ONRC template by procedure ID
 */
export function getONRCTemplateByProcedureId(procedureId: string): MapaTemplate | undefined {
  return ONRC_TEMPLATES.find((t) => t.id === `onrc-${procedureId}` || t.id === procedureId);
}

/**
 * Get ONRC template by ID
 */
export function getONRCTemplateById(id: string): MapaTemplate | undefined {
  return ONRC_TEMPLATES.find((t) => t.id === id);
}

/**
 * Get templates by category
 */
export function getONRCTemplatesByCategory(categoryId: string): MapaTemplate[] {
  const defs = ONRC_TEMPLATE_DEFINITIONS.filter((d) => d.categoryId === categoryId);
  return defs.map(toMapaTemplate);
}
