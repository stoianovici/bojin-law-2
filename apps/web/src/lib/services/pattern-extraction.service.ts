...
export class PatternExtractionService {
  ...
  async generateTemplateStructure(
    ...
    if (!hasVariables && documents.length > 0) {
      ...
      if (fallbackVariables.length > 0 && enrichedSections.length > 0) {
        // Add variables to the first section as a fallback
        enrichedSections[0].variablePlaceholders = fallbackVariables;
      }
    }
  ...
  private extractVariablePlaceholders(sectionContent: string[]): string[] {
    ...
    const patterns = [
      ...
      /\b(\d{1,3}(?:[,.]\d{3})*(?:[,.]\d{1,2})?\s+(?:RON|EUR|lei))\b/g, // Amounts: "10,000 RON"
