...
function TaskCard({ task, onClick, isDragging = false }: TaskCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Case name */}
      {caseName && (
        <p className="text-xs text-linear-text-tertiary mb-2 line-clamp-1">{caseName}</p>
      )}
...
function TaskCardOverlay({ task }: { task: Task }) {
  ...
  return (
    ...
    >
      ...
      {caseName && (
        <p className="text-xs text-linear-text-tertiary mt-1 line-clamp-1">{caseName}</p>
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-bold text-linear-text-secondary uppercase tracking-wide">
          {column.title}
        </h2>
        <span
