...
- [2025-12-15] Session 8: **MAJOR BREAKTHROUGH** - Diagnostics revealed attachments exist in EmailAttachment table (55 total across 9 emails) but all have `attachmentsAlreadyExist` status. They were synced in a previous import but WITHOUT Document/CaseDocument records because emails weren't linked to case at sync time.

- [2025-12-15] Session 8: Implemented fix to "upgrade" existing attachments by re-downloading from Graph API and creating Document/CaseDocument records. Deployed commit `3ec7d36`.

- [2025-12-15] Session 8: Fix deployed but `attachmentsImported` still shows 0 in UI. Need to check if upgrade is encountering errors (possibly OneDrive or Graph API issues). Expand `attachmentSyncDetails` in console to see per-email errors.
...
- [2025-12-15] Session 9 started. Continuing from: Fixing (upgrade deployed but not working)

- [2025-12-15] Session 9: Added enhanced diagnostics to identify why upgrade logic isn't working:
  - New `emailCaseId` field in `AttachmentSyncDetail` - shows email's caseId at sync time
  - New `upgradedWithDocument` counter - tracks successful upgrades
  - Detailed logging at upgrade decision point with `willUpgrade` flag
  - Hypothesis: `email.caseId` might be null when fetched even after updateMany

- [2025-12-15] Session 9: Deployed commit `7ac1e5f`. Need to test and check `emailCaseId` in console output.

- [2025-12-15] Session 10 started. Continuing from: Fixing (enhanced diagnostics deployed)

- [2025-12-15] Session 10: Tested diagnostics - `emailCaseId` IS set (not null), but `upgradedWithDocument: 0`. This disproved the timing hypothesis.

- [2025-12-15] Session 10: **ROOT CAUSE CONFIRMED** - EmailAttachment records have `documentId` set but pointing to non-existent Documents (orphaned references). The upgrade condition `!existing.documentId` was false because documentId was set, even though the Document didn't exist.

- [2025-12-15] Session 10: Implemented fix to check if Document actually exists before deciding to upgrade:

  ```typescript
  // Check if Document actually exists (could be orphaned reference)
...
- [2025-12-15] Session 10: Added `orphanedDocumentIds` diagnostic field to track attachments with orphaned document references.

- [2025-12-15] Session 10: Deployed commit `26c6ca3`. Gateway building - awaiting verification.
