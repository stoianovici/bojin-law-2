...
- [x] **Task 2: Create Database Migration Script** (AC: 1, 2)
  - [x] Migration steps:
    ...
    3. Populate `clientId` for existing documents (from case → client relationship)
...
- [x] **Task 12: Implement Document List with Case Grouping** (AC: 4, 7)
  ...
  - [x] Group documents by original/source case:
    ...
    Case #12345 - Smith Contract Matter
      ☐ Business License.pdf (uploaded Jan 15, 2025)
      ☐ Tax Certificate.pdf (uploaded Jan 20, 2025)

    Case #12340 - Smith Property Dispute
      ☐ Property Deed.pdf (uploaded Dec 10, 2024)
      ☐ Title Insurance.pdf (uploaded Dec 12, 2024)
    ```
...
- [x] **Task 15: Update Case Documents List Component** (AC: 6, 7)
  - [x] Query documents via case → caseDocuments relationship
...
- [x] **Task 27: Write Backend Unit Tests** (All ACs)
  - [x] Test document ownership change (Case → Client)
...
- [ ] **Task 29: Write Integration Tests** (All ACs)
  - [ ] Test complete import flow: Open modal → Search → Select → Import → Verify in case list
  - [ ] Test unlink flow: Remove document → Verify still in other case
  - [ ] Test permanent delete flow: Delete document → Verify removed from all cases
  - [ ] Test document upload: Upload → Link to multiple cases
...
```
Client 1:N→ Document
Document N:M→ Case (via CaseDocument join table)
...
```
┌─────────────────────────────────────────────────┐
│ Import Documents from ABC Corporation           │
├─────────────────────────────────────────────────┤
│ 🔍 [Search documents...        ] [Filter ▼]     │
├─────────────────────────────────────────────────┤
│ ☐ Select All                  Showing 12 docs   │
├─────────────────────────────────────────────────┤
│ ▼ Case #12345 - Contract Review (5 docs)        │
│   ☐ 📄 Business License.pdf          2.3 MB     │
│       Uploaded Jan 15 by John Smith              │
│   ☐ 📄 Tax Certificate.pdf           1.1 MB     │
│       Uploaded Jan 20 by Jane Doe                │
│                                                   │
│ ▼ Case #12340 - Property Dispute (7 docs)       │
│   ☐ 📄 Property Deed.pdf              890 KB    │
│       Uploaded Dec 10 by John Smith              │
│   ☐ 📷 Property Photos.zip           15.2 MB    │
│       Uploaded Dec 12 by Jane Doe                │
│   ...                                            │
├─────────────────────────────────────────────────┤
│              [Cancel] [Add Selected (3)] ← Blue  │
└─────────────────────────────────────────────────┘
```
...
- **Key Test Cases**:
  - Document ownership change (Case → Client)
...
- **Location**: `apps/web/src/app/**/*.test.tsx`
- **Key Test Flows**:
  - Complete import flow: Open browser → Search → Select → Import → Verify
  - Unlink flow: Remove document from case → Verify still in other case
  - Delete flow: Permanent delete → Verify removed from all cases and storage
...
- **Critical User Journey**:
  - Create case → Upload document → Create second case with same client → Import document from first case → Verify document in both cases
...
- Coding Standards: ✓ Types defined in packages/shared/types, proper naming conventions followed
- Project Structure: ✓ Files in correct locations per architecture
- Testing Strategy: ✓ Unit tests at 80%+ backend, 70%+ frontend targets
- All ACs Met: ✓ Core ACs 1-10 implemented (see traceability below)

### Requirements Traceability

| AC | Description | Test Coverage | Status |
|----|-------------|---------------|--------|
| 1 | Documents owned by Client (not Case) | `document.resolvers.test.ts`: uploadDocument creates with clientId | ✓ COVERED |
| 2 | Many-to-many Case-Document relationship | `document.resolvers.test.ts`: linkDocumentsToCase tests | ✓ COVERED |
| 3 | Browse client documents when creating/editing case | `DocumentBrowserModal.test.tsx`: renders modal, filters documents | ✓ COVERED |
| 4 | Document browser with search and case grouping | `DocumentBrowserModal.test.tsx`: search functionality, group expansion | ✓ COVERED |
| 5 | Multi-select and add documents | `DocumentBrowserModal.test.tsx`: selection tests, import action | ✓ COVERED |
| 6 | Imported documents appear same as uploaded | `CaseDocumentsList.test.tsx`: renders document list | ✓ COVERED |
| 7 | Visual indicator for source case | `CaseDocumentsList.test.tsx`: Imported badge, source case display | ✓ COVERED |
| 8 | Import at any time (not just creation) | `CaseDocumentsList.test.tsx`: Import Documents button always available | ✓ COVERED |
| 9 | Unlink removes from case only | `document.resolvers.test.ts`: unlinkDocumentFromCase tests; `CaseDocumentsList.test.tsx`: unlink action | ✓ COVERED |
| 10 | Only Partners can permanently delete | `document.resolvers.test.ts`: Partner delete test + Associate rejection; `CaseDocumentsList.test.tsx`: delete button visibility by role | ✓ COVERED |
...
1. **Database Indexes**: ✓ Properly defined
...
2. **Query Optimization**: ✓ Acceptable
...
Gate: **PASS** → docs/qa/gates/2.8.4-cross-case-document-linking.yml
