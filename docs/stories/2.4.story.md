...
describe('AuthService', () => {
  ...
  describe('exchangeCodeForTokens', () => {
    ...
    it('should throw error if code exchange fails', async () => {
      ...
      await expect(authService.exchangeCodeForTokens('invalid-code', 'verifier')).rejects.toThrow(
        'Invalid code'
      );
    });
  });
...
- services/gateway/**tests**/integration/token-refresh.integration.test.ts - Added 3 error handling tests
- services/gateway/**tests**/integration/logout.integration.test.ts - Added 2 error handling tests
- services/gateway/**tests**/integration/rate-limiting.integration.test.ts - Created with 6 tests
...
**Post-MVP Enhancements:** 3. Consider implementing token blacklist for immediate revocation (LOW priority) 4. Extract token expiry calculation helper (LOW priority)
