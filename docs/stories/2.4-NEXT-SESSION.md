...
├── config/auth.config.ts          ← Azure AD & MSAL config
├── services/
│   ├── auth.service.ts            ← OAuth flow (complete)
│   └── jwt.service.ts             ← JWT tokens (complete)
├── middleware/auth.middleware.ts  ← JWT validation (complete)
├── routes/auth.routes.ts          ← Auth endpoints (needs updates)
└── types/auth.types.ts            ← TypeScript types (complete)
```
...
services/gateway/src/
└── services/
    └── user.service.ts            ← CREATE THIS (Task 8)

packages/database/
└── prisma/
    ├── schema.prisma              ← UPDATE THIS (Task 7)
    └── seed.ts                    ← UPDATE THIS (Task 7)
```
...
**Solution:** Set all AZURE*AD*\* env vars, even for testing (use dummy values)
...
✅ Azure AD authorization URL creation
...
✅ Active user requirement middleware

## What Doesn't Work Yet

❌ User creation/retrieval from database (no User model)
❌ User status validation (no User model)
❌ Session storage in Redis (waiting for user data)
❌ Complete OAuth callback endpoint (needs user provisioning)
❌ Token refresh endpoint (needs session storage)
❌ Logout endpoint (needs session storage)
❌ Frontend integration (no login page yet)
