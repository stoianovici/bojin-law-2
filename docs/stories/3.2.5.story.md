...
**Deployment Model:** Web application hosted at `import.yourapp.com` via Render.com with **secure server-side PST processing**. PST files are uploaded to cloud storage (Cloudflare R2 or Azure Blob Storage, encrypted AES-256), processed server-side, then automatically deleted after categorization completes. Only categorized documents are exported to OneDrive.

**Note:** Storage provider (Cloudflare R2 vs Azure Blob Storage) to be finalized. Render.com infrastructure replaces Azure App Service (Story 2.1.1).
...
- ✅ **Uploads to Azure Blob Storage with server-side encryption (AES-256)**
...
- ✅ Stores extracted documents in temporary Azure Blob (encrypted)
...
- ✅ **CRITICAL: Automatically deletes original PST file from Azure Blob**
- ✅ **Deletes all temporary extracted documents from Azure Blob**
...
| **Temporary Storage** | Cloudflare R2 or Azure Blob Storage      | Encrypted at rest (AES-256), secure, scalable, S3-compatible  |
...
| **Authentication**    | Azure AD (existing)                      | Partner + Assistant roles, SSO                                |
