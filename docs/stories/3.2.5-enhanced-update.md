...
- ✅ **Detects primary language** (Romanian/English/Mixed) with confidence scoring
...
- ✅ **Extracts key legal terms** in both languages
...
- ✅ **Filter by primary language**: Romanian Only / English Only / Mixed
   ...
   export class AIDocumentAnalyzer {
     ...
     async analyzeDocumentBatch(documents: Document[]): Promise<AnalysisResult[]> {
       ...
       const results = await Promise.all(
         batches.map(batch => this.analyzeWithClaude(batch))
       );
...
1. **Add language badges to document list:**
   ...
   <DocumentCard>
     <div className="flex gap-2 mb-2">
       ...
       {doc.templatePotential === 'High' && (
         <Badge variant="success">Template Ready</Badge>
       )}
     </div>
   </DocumentCard>
   ```
...
describe('AIDocumentAnalyzer', () => {
  ...
  it('should respect cost limits', async () => {
    ...
    expect(cost).toBeLessThan(10.00);
  });
});
```
